...
def upgrade():
    ...
    company_type = sa.Enum("PUBLIC", "PRE_IPO", "PRIVATE", "UNKNOWN", name="companytype")
    lifecycle = sa.Enum(
        "ACTIVE", "INACTIVE", "REJECTED", "EXCLUDED", "PENDING_REVIEW", name="companylifecycle"
    )
    review_status = sa.Enum(
        "NOT_REVIEWED", "REJECTED", "INTERESTED", "APPLIED", name="reviewstatus"
    )
    career_status = sa.Enum(
        "ACTIVE", "REMOVED", "UNKNOWN", "SCAN_FAILED", "UNSUPPORTED", name="careersitestatus"
    )
    workflow_status = sa.Enum(
        "PENDING", "RUNNING", "SUCCEEDED", "FAILED", name="workflowstatus"
    )
    ...
    op.create_table(
        ...
        sa.Column("lifecycle_state", lifecycle, nullable=False),
        ...
        sa.Column("active_for_scanning", sa.Boolean(), nullable=False, server_default=sa.true()),
        ...
        sa.Column("last_scan_status", sa.String(length=100), nullable=True),
        sa.Column("last_scan_error", sa.Text(), nullable=True),
        sa.Column("seed_source", sa.String(length=255), nullable=True),
        sa.Column("seed_prompt_version", sa.String(length=255), nullable=True),
    ...
    op.create_index("ix_companies_lifecycle_state", "companies", ["lifecycle_state"])
    ...
    op.create_index(
        "ix_job_postings_job_description_hash",
        "job_postings",
